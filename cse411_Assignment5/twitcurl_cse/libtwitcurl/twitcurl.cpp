...
bool twitCurl::search( const std::string& searchQuery, const std::string resultCount )
{
    ...
    if( resultCount.size() )
    {
        buildUrl = buildUrl.substr(0,buildUrl.size()-1);
        buildUrl += twitCurlDefaults::TWITCURL_URL_SEP_AMP + twitCurlDefaults::TWITCURL_COUNT + urlencode( resultCount );
        buildUrl = buildUrl.substr(0,buildUrl.size()-3);
        printf("%s\n", buildUrl.c_str());
    }
